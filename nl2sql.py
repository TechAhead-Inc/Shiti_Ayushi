...
GENERATED POSTGRESQL QUERY:"""
        ...
        try:
            ...
            if result_count == 1 and len(results[0]) == 1:
                ...
                if 'max' in key.lower() or 'MAX(' in sql_query.upper():
                    return f"⬆️ **Maximum Value**: {value:,} (highest value found)"
                
                if 'min' in key.lower() or 'MIN(' in sql_query.upper():
                    return f"⬇️ **Minimum Value**: {value:,} (lowest value found)"
    ...
    def suggest_visualization_options(self, results: List[Dict[str, Any]], natural_query: str) -> List[Dict[str, Any]]:
        """Enhanced visualization suggestions with detailed recommendations."""
        if not results:
            return []
        ...
        for col in columns:
            ...
            # Check if date/time related
            elif any(keyword in str(sample_values[0]).lower() for keyword in ['date', 'time', 'year', 'month']) or any(keyword in col.lower() for keyword in ['date', 'time', 'created', 'updated']):
        ...
        if text_columns and numeric_columns and row_count <= 50:
            unique_categories = len(set(str(row[text_columns[0]]) for row in results))
            if unique_categories <= 20:
                suggestions.append({
                    'type': 'bar',
                    'title': '📊 Bar Chart',
                    'description': f'Compare {numeric_columns[0]} across {text_columns[0]} categories',
                    'priority': 2,
        ...
        if date_columns and numeric_columns:
            suggestions.append({
                'type': 'line',
                'title': '📈 Line Chart',  
                'description': f'Show trends of {numeric_columns[0]} over {date_columns[0]}',
                'priority': 2,
        ...
        if len(numeric_columns) >= 2:
            suggestions.append({
                'type': 'scatter',
                'title': '🔍 Scatter Plot',
                'description': f'Explore relationship between {numeric_columns[0]} and {numeric_columns[1]}',
                'priority': 3,
                'suitable_for': 'Finding correlations between numeric variables'
            })

        # Pie chart for distributions
        if text_columns and numeric_columns and row_count <= 15:
            unique_categories = len(set(str(row[text_columns[0]]) for row in results))
        ...
        if numeric_columns and row_count > 20:
            suggestions.append({
                'type': 'histogram',
                'title': '📊 Histogram',
                'description': f'Show distribution pattern of {numeric_columns[0]}',
                'priority': 4,
